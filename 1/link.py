# main
iconLink = './textures/icon.png'
textFolder = './textures/text/'

# menu
freddyMainLink = './textures/Main/'
firstNoiseLink = './textures/Noise/first/'
secondNoiseLink = './textures/Noise/second/'
buttonsLink = './textures/buttons/menu/'
nightCounterLink = './textures/nights/'
# ~sounds
menuMusicLink = './sounds/main/main_theme.mp3'
mainSoundsFolder = './sounds/'

# night main
officeFolder = './textures/office/'
fanFolder = './textures/Fan/'
buttonLeftFolder = './textures/buttons/Left/'
buttonRightFolder = './textures/buttons/Right/'
doorLeftFolder = './textures/doors/Left Door/'
doorRightFolder = './textures/doors/Right Door/'
doorLeftCloseFolder = './textures/doors/Left Door Close/'
doorRightCloseFolder = './textures/doors/Right Door Close/'
clockFolder = './textures/clock/'
batteryFolder = './textures/battery/'